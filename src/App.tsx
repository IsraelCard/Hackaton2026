...
import { ZONAS, predecir, PredictionResult } from './constants';
...
export default function App() {
  ...
  const [result, setResult] = useState<{ 
    analysis: AIResponse, 
    data: PredictionResult & { incidentes: any[], fecha_consulta: string } 
  } | null>(null);
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col items-center transition-colors duration-300">
        ...
        <div className="lg:col-span-8 flex flex-col gap-8">
          <AnimatePresence mode="wait">
            {!result && !loading ? (
              ...
              <AnalysisResults result={result} setIsMapExpanded={setIsMapExpanded} />
            )}
          </AnimatePresence>
        </div>
      </main>
      ...
      <footer className="w-full max-w-7xl mt-12 py-12 border-t border-zinc-200 dark:border-zinc-800 px-6 flex flex-col md:flex-row justify-between items-center gap-6 transition-colors">
        ...
        <div className="flex gap-8">
           {['TomTom Flow', 'Gemini AI', 'Data Status'].map((f) => (
             <a key={f} href="#" className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 hover:text-blue-600 transition-colors uppercase tracking-[0.2em]">{f}</a>
           ))}
        </div>
      </footer>
    </div>
